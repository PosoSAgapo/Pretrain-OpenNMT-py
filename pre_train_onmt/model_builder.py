...
from pre_train_onmt.encoders import str2enc

from pre_train_onmt.decoders import str2dec

from pre_train_onmt.modules import Embeddings, CopyGenerator
from pre_train_onmt.modules.util_class import Cast
...
def build_task_specific_model(model_opt, fields):
    ...
    if model_opt.model_task == ModelTask.SEQ2SEQ:
        ...
        return pre_train_onmt.models.NMTModel(encoder=encoder, decoder=decoder)
    elif model_opt.model_task == ModelTask.LANGUAGE_MODEL:
        src_emb = build_src_emb(model_opt, fields)
        decoder, _ = build_decoder_with_embeddings(
            model_opt, fields, share_embeddings=True, src_emb=src_emb
        )
        return pre_train_onmt.models.LanguageModel(decoder=decoder)
    else:
...
def build_base_model(model_opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    if not model_opt.copy_attn:
        if model_opt.generator_function == "sparsemax":
            gen_func = pre_train_onmt.modules.sparse_activations.LogSparsemax(dim=-1)
        else:
