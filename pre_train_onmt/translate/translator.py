...
import pre_train_onmt.inputters as inputters
import pre_train_onmt.decoders.ensemble
from pre_train_onmt.inputters.text_dataset import InferenceDataIterator
from pre_train_onmt.translate.beam_search import BeamSearch, BeamSearchLM
from pre_train_onmt.translate.greedy_search import GreedySearch, GreedySearchLM
from pre_train_onmt.utils.misc import tile, set_random_seed, report_matrix
from pre_train_onmt.utils.alignment import extract_alignment, build_align_pharaoh
from pre_train_onmt.modules.copy_generator import collapse_copy_scores
from pre_train_onmt.constants import ModelTask
...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    load_test_model = (
        pre_train_onmt.decoders.ensemble.load_test_model
        if len(opt.models) > 1
        else pre_train_onmt.model_builder.load_test_model
    )
    fields, model, model_opt = load_test_model(opt)

    scorer = pre_train_onmt.translate.GNMTGlobalScorer.from_opt(opt)
...
class Inference(object):
    ...
    def _translate(
        ...
        xlation_builder = pre_train_onmt.translate.TranslationBuilder(
            data,
            self.fields,
            self.n_best,
            self.replace_unk,
